...
spa.model = (function () {
	...
	people = (function  () {
		...
		logout = function () {
			...
			$.gevent.puhlish( 'spa-logout', [ user ] );
			return is_removed;
		};
...
//chatオブジェクト
	chat = ( function () {
		...
		_update_list = function ( arg_list ){
			...
			//チャット結果がオンラインでなくなっている場合はチャット相手を解除する。その結果 spa-setchateeグローバルイベントが発行される。
		...
		join_chat = function () {
			...
			if ( stateMap.user.get_is_anon() ){
				console.warn( 'User must be defined before joing chat' );
				return false;
			}
		...
		send_msg = function ( msg_text ) {
			...
			//updatechatを発行したので送信メッセージを表示できる。
		...
		//{ person_id : <string>, css_map : { top : <int>, left : <int>, 'background-color' : <string> } };
